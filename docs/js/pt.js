...
Util = (function() {
  ...
  Util.RK4 = function(c, d, func, dt, t) {
  ...
  Util.bestCandidate = function(bound, items, samples) {
    var best, bestDist, halfsize, i, maxDist, nearest, p, quartersize, size, _i, _nearest;
    if (samples == null) {
      samples = 10;
    }
    size = bound.size();
...
Space = (function() {
  ...
  Space.prototype.add = function(item) {
    ...
    if ((item.animate != null) && typeof item.animate === 'function') {
      ...
      this.items[k] = item;
      item.animateID = k;
      if (item.spaceResize != null) {
        item.spaceResize(this.size.x, this.size.y);
      }
    } else {
...
Vector = (function(_super) {
  ...
  Vector.prototype.moveTo = function() {
    var d, p, pts, target, _i, _len;
  ...
  Vector.prototype.moveBy = function() {
    var inc, p, pts, _i, _len;
...
Pair = (function(_super) {
  ...
  Pair.prototype.distance = function(sqrt) {
    var d, dx, dy, dz;
    if (sqrt == null) {
      sqrt = true;
    }
...
Line = (function(_super) {
  ...
  Line.prototype.subpoints = function(num) {
    var t, _i, _results;
    _results = [];
    for (t = _i = 0; 0 <= num ? _i <= num : _i >= num; t = 0 <= num ? ++_i : --_i) {
      _results.push(this.interpolate(t / num));
    }
    return _results;
  };
...
Rectangle = (function(_super) {
  ...
  Rectangle.prototype.intersectLine = function(line, get_pts) {
    var ip1, ip2, lbound, p, pts, s, sides, _i, _len;
    if (get_pts == null) {
      get_pts = true;
    }
  ...
  Rectangle.prototype.intersectRectangle = function(rect, get_pts) {
    var intersected, p, pts, sa, sb, sidesA, sidesB, xi, yi, zi, _i, _j, _len, _len1;
    if (get_pts == null) {
      get_pts = true;
    }
    ...
    sidesB = rect.sides();
    pts = [];
    for (_i = 0, _len = sidesA.length; _i < _len; _i++) {
      sa = sidesA[_i];
      for (_j = 0, _len1 = sidesB.length; _j < _len1; _j++) {
        sb = sidesB[_j];
...
Form = (function() {
  ...
  Form.lines = function(ctx, pairs) {
    var ln, _i, _len, _results;
    _results = [];
    for (_i = 0, _len = pairs.length; _i < _len; _i++) {
      ln = pairs[_i];
      _results.push(Form.line(ctx, ln));
    }
    return _results;
  };
  ...
  Form.points = function(ctx, pts, halfsize, fill, stroke, circle) {
    ...
    for (_i = 0, _len = pts.length; _i < _len; _i++) {
      p = pts[_i];
      _results.push(Form.point(ctx, p, halfsize, fill, stroke, circle));
    }
    return _results;
  };
  ...
  Form.prototype.stroke = function(c, width) {
...
Particle = (function(_super) {
  ...
  Particle.collideParticle2d = function(pa, pb, update) {
    var d1, d2, dot1n, dot1t, dot2n, dot2t, normal, pav, pbv, tangent, v1n, v1t, v2n, v2t;
    if (update == null) {
      update = true;
    }
  ...
  Particle.force_gravitation = function(state, t, pa, pb) {
    var d, force, mag, meterToPixel;
    meterToPixel = 30;
    d = pb.$subtract(state.position);
    mag = d.magnitude() / meterToPixel;
    force = mag === 0 ? 0 : t * 0.00667 * pa.mass * pb.mass / (mag * mag);
...
ParticleSystem = (function() {
  ...
  ParticleSystem.prototype.add = function(particle) {
    particle.id = this.particles.length;
  ...
  ParticleSystem.prototype.animate = function(time, frame, ctx) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      p = _ref[_i];
      if (p.life.active) {
        _results.push(p.animate(time, frame, ctx));
      } else {
...
Pt.ParticleSystem = ParticleSystem;
...
Grid = (function(_super) {
  ...
  Grid.prototype.generate = function(callback) {
    var c, cell, pos, r, _i, _j, _ref, _ref1;
  ...
  Grid.prototype.occupy = function(x, y, w, h) {
    var c, r, _i, _j;
    for (c = _i = 0; 0 <= w ? _i < w : _i > w; c = 0 <= w ? ++_i : --_i) {
      for (r = _j = 0; 0 <= h ? _j < h : _j > h; r = 0 <= h ? ++_j : --_j) {
  ...
  Grid.prototype.fit = function(cols, rows) {
    var b, cell, colCount, colSize, currCol, currRow, freeCol, _i, _j, _ref, _ref1;
    colSize = Math.min(cols, this.columns);
    for (currRow = _i = 0, _ref = this.rows; 0 <= _ref ? _i < _ref : _i > _ref; currRow = 0 <= _ref ? ++_i : --_i) {
      colCount = colSize;
      freeCol = 0;
      for (currCol = _j = 0, _ref1 = this.columns; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; currCol = 0 <= _ref1 ? ++_j : --_j) {
...
PointSet = (function(_super) {
  ...
  PointSet.prototype.sides = function(close_path) {
    var lastP, p, sides, _i, _len, _ref;
    if (close_path == null) {
      close_path = true;
    }
    lastP = null;
  ...
  PointSet.prototype.angles = function(axis) {
    var angles, i, v1, v2, _i, _ref;
    if (axis == null) {
      axis = Const.xy;
    }
    angles = [];
    for (i = _i = 1, _ref = this.points.length - 1; _i < _ref; i = _i += 1) {
...
CanvasSpace = (function(_super) {

  __extends(CanvasSpace, _super);

  CanvasSpace.name = 'CanvasSpace';

  function CanvasSpace(id, context, bgcolor) {
    if (id == null) {
      id = 'space';
    }
    if (context == null) {
    ...
    if (!this.appended) {
      ...
      if (frame) {
        ...
        window.addEventListener('resize', (function() {
          frame_rect = frame.getBoundingClientRect();
          return this.resize(frame_rect.width, frame_rect.height);
        }).bind(this));
  ...
  CanvasSpace.prototype.resize = function(w, h) {
    var k, p, _ref;
    this.size.set(w, h);
  ...
  CanvasSpace.prototype._mouseAction = function(type, evt) {
    ...
    for (k in _ref) {
      ...
      if (v.onMouseAction != null) {
        _results.push(v.onMouseAction(type, evt.offsetX, evt.offsetY, evt));
      } else {
        _results.push(void 0);
      }
    }
...
Triangle = (function(_super) {
  ...
  Triangle.prototype.perimeter = function() {
    var lens, sides;
    sides = this.sides();
    lens = [sides[0].distance(), sides[1].distance(), sides[2].distance()];
    return {
      sides: sides,
